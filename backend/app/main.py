...
@app.post("/v1/analyze")
async def analyze(file: UploadFile=File(...), segment_length_m: float=settings.segment_length_m):
    ...
    segments=[]
    for i,(a,b) in enumerate(pairs): segments.append(classify(i,a,b,nearest_way(a,ways)))
    ...
    return {"name":name,"profile":"VL standard","warning":"Analyse indicative : une reconnaissance terrain reste indispensable.","source_status":"online" if ways else "degraded","statistics":{"distance_m":round(total,1),"ascent_m":round(ascent,1),"elevation_min_m":min(elevations) if elevations else None,"elevation_max_m":max(elevations) if elevations else None,"by_class_m":{k:round(v,1) for k,v in by_class.items()},"uncertain_percent":round(100*by_class["gray"]/sum(by_class.values()),1) if segments else 100},"geojson":geojson}

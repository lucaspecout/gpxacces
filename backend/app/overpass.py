...
TAGS = ("highway","surface","smoothness","tracktype","width","access","vehicle","motor_vehicle","emergency","barrier","incline","ford","bridge","tunnel","service")
...
async def fetch_ways(points: list[Point]) -> list[dict]:
    south=min(p.lat for p in points)-.001; north=max(p.lat for p in points)+.001
    west=min(p.lon for p in points)-.001; east=max(p.lon for p in points)+.001
    ...
    for url in settings.overpass_urls.split(","):
        try:
            async with httpx.AsyncClient(timeout=settings.overpass_timeout_seconds) as client:
                response=await client.post(url.strip(), content=query, headers={"User-Agent":"GPXAccess/0.1"})
